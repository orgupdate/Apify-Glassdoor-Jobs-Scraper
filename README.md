...
The most efficient way to extract job listings directly from Glassdoor Jobs search results.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single Glassdoor search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the Glassdoor Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "Glassdoor", "Microsoft") |
    ...
    [
      {
        ...
        "date": "2025-03-25",
        "job_type": "Full-time",
        "URL": "https://www.glassdoor.com/search?..."
      }
    ]
...
- **posted_via:** The original platform where Glassdoor found the job (e.g., Monster, Greenhouse, LinkedIn).
...
### Python
    ...
    run = client.actor("orgupdate/glassdoor-jobs-scraper").call(run_input=run_input)
...
### Node.js
    ...
    (async () => {
        // Run the Actor and wait for it to finish
        const run = await client.actor('orgupdate/glassdoor-jobs-scraper').call(input);
...
- **Glassdoor Sheets:** Automatically save new job listings into a spreadsheet for analysis.
